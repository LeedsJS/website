    ...
    get nextEvent() {
      ...
      const keys = Object.keys(this.data)
        .filter((key) => {
          ...
          return (
            now.isSameOrAfter(event.announce_date) &&
            now.isSameOrBefore(dayAfterEvent, "day")
          );
        })
        .sort((a, b) => {
...
function addMethods(event) {
  return Object.assign({}, event, {
    ticketsAvailable() {
      ...
      return (
        now.isSameOrAfter(this.ticket_date) &&
        now.isSameOrBefore(this.date, "day")
      );
    },
